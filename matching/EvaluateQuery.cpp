...
EvaluateQuery::exploreCECIStyle(const Graph *data_graph, const Graph *query_graph, TreeNode *tree, ui **candidates,
    ...
    while (true) {
        ...
        */
        ...
        while (idx[cur_depth] < idx_count[cur_depth]) {//遍历当前搜索树结点的候选节点
            /*
            block1.1------------------------------------------------
            */
            VertexID u = order[cur_depth];
            ...
            */
            ...
            reverse_embedding[v] = u;
#endif
            if (cur_depth == max_depth - 1) {//找到一个匹配的结果
                embedding_cnt += 1;
                visited_vertices[v] = false;
#ifdef ENABLE_FAILING_SET
                reverse_embedding.erase(embedding[u]);
                ...
                vec_failing_set[cur_depth - 1] |= vec_failing_set[cur_depth];
#endif
                if (embedding_cnt >= output_limit_num) {//找到指定数量的子图，查询结束
                    goto EXIT;
                }
            } else {//生成候选节点
    ...
    // printf("时间1 (seconds): %.4lf\n", NANOSECTOSEC(std::chrono::duration_cast<std::chrono::nanoseconds>(time1).count()));
    // printf("时间2 (seconds): %.4lf\n", NANOSECTOSEC(std::chrono::duration_cast<std::chrono::nanoseconds>(time2).count()));
...
void EvaluateQuery::generateValidCandidates(ui depth, ui *embedding, ui *idx_count, ui **valid_candidates, ui *order,
    ...
    idx_count[depth] = valid_candidates_count;//候选节点个数
}
